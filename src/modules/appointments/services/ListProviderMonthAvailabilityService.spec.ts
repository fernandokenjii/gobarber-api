...
describe('ListMonthAvailability', () => {
  ...
  it('should be able to list the month availability of provider', async () => {
    await fakeAppointmentsRepository.create({
      provider_id: 'user',
      date: new Date(2020, 5, 5, 8, 0, 0),
    });

    await fakeAppointmentsRepository.create({
      provider_id: 'user',
      date: new Date(2020, 5, 5, 9, 0, 0),
    });

    await fakeAppointmentsRepository.create({
      provider_id: 'user',
      date: new Date(2020, 5, 5, 10, 0, 0),
    });
    ...
    await fakeAppointmentsRepository.create({
      provider_id: 'user',
      date: new Date(2020, 5, 5, 16, 0, 0),
    });

    await fakeAppointmentsRepository.create({
      provider_id: 'user',
      date: new Date(2020, 5, 5, 17, 0, 0),
    });
